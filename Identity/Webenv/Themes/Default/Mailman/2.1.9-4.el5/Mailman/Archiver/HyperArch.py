...
class Article(pipermail.Article):
    ...
    def as_html(self):
        ...
        try:
            ...
            d["listadmin_link"] = Link(Utils.ScriptURL('admin'), _('Administration')).Format()
            d["listinfo_link"] = Link(Utils.ScriptURL('listinfo'), _('General Information')).Format()
            d["errormsg_header"] = _('Mailing Lists')
...
class HyperArchive(pipermail.T):
    ...
    def html_TOC(self):
		...
		highlight = 1
			...
			css = ''
			highlight = 1
                    accum.append(self.html_TOC_entry(a, css))
    ...
    def html_TOC_entry(self, arch, css):
...
# The following two methods should be inverses of each other. -ddm
        ...
        for i in xrange(0, len(source)):
            ...
            if self.IQUOTES:
                ...
                if quoted is None:
                    ...
                    quoted = quoted.end(0)
                    prefix = CGIescape(L[:quoted], self.lang) + '<em>'
                    suffix = '</em>'
                    if self.SHOWHTML:
                        suffix += '<br />'
    ...
    def format_article(self, article):
        ...
        if not self.SHOWHTML and lines:
            lines.insert(0, '<pre class="article">')
            lines.append('</pre>')
        else:
