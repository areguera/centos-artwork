...
<?php 
	...
	if ( $f_error ) {
		...
		print '<p>' . lang_get( 'login_error' ) . '</p>';
		print '</div>';
	}

	if ( $f_cookie_error ) {
		print '<div id="message" class="red">';
		print '<p>' . lang_get( 'login_cookies_disabled' ) . '</p>';
		print '</div>';
	}

?>

<h2><?php echo lang_get( 'login_title' ); ?></h2>
	...
	<table>
		...
		<tr>
			...
			<td style="text-align:right">
				<strong><?php echo lang_get( 'password' ) ?>:</strong>
			</td>
		...
		<tr>
			...
			<td style="text-align:right">
				<strong><?php echo lang_get( 'save_login' ) ?>:</strong>
			</td>
...
<?php
	...
	if ( config_get( 'login_method' ) === PLAIN ) {
		print '<p class="lmtxt red"><strong>WARNING:</strong> Plain password authentication is used, this will expose your passwords to administrators.</p>';
	}
	...
	if ( $t_admin_user_id !== false ) {
		if ( user_is_enabled( $t_admin_user_id ) && auth_does_password_match( $t_admin_user_id, 'root' ) ) {
			print '<p class="lmtxt red"><strong>WARNING:</strong> You should disable the default "administrator" account or change its password.</p>';
		}
	}
	...
	if ( 0 && is_dir( $t_admin_dir ) && is_readable( $t_admin_dir ) ) {
		print '<p class="lmtxt red"><strong>WARNING:</strong> Admin directory should be removed.</p>' . "\n";
		...
		if ( $t_db_version == 0 ) {
			...
			if ( ( $t_upgrade_count != $t_upgrades_reqd ) &&
				...
				print '<p class="lmtxt red"><strong>WARNING:</strong> The database structure may be out of date. Please upgrade <a href="admin/upgrade.php">here</a> before logging in.</p>';
			}
		}
		...
		if ( ( 0 < $t_db_version ) &&
			...
			if ( $t_db_version < $t_upgrades_reqd ) {
				print '<p class="lmtxt red"><strong>WARNING:</strong> The database structure may be out of date. Please upgrade <a href="admin/install.php">here</a> before logging in.</p>';
			} else {
				print '<p class="lmtxt red"><strong>WARNING:</strong> The database structure is more up-to-date than the code installed.  Please upgrade the code.</p>';
			}
		}
	}
?>
