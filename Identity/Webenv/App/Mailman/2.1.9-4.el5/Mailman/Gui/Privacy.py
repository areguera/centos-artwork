    ...
    def GetConfigInfo(self, mlist, category, subcat=None):
        ...
        if mm_cfg.ALLOW_OPEN_SUBSCRIBE:
            sub_cfentry = ('subscribe_policy', mm_cfg.Radio,
			   ...
			   <ul>
			   	<li>''' + _('None - no verification steps (<em>Not Recommended</em>)') + '''</li>
                           	<li>''' + _('Confirm (*) - email confirmation step required') + '''</li>
                           	<li>''' + _('Require approval - require list administrator approval for subscriptions') + '''</li>
                           	<li>''' + _('Confirm and approve - both confirm and approve') + '''</li>
                           
			   </ul>

                           <p>''' + _('(*) when someone requests a subscription, Mailman sends them a notice with a unique subscription request number that they must reply to in order to subscribe. This prevents mischievous (or malicious) people from creating subscriptions for others without their consent.'))
        else:
            sub_cfentry = ('subscribe_policy', mm_cfg.Radio,
                           # choices
                           (_('Confirm'),
        ...
        subscribing_rtn = [
            '<p>' + _("""This section allows you to configure subscription and membership exposure policy. You can also control whether this list is public or not. See also the <a href="%(admin)s/archive">Archival Options</a> section for separate archive-related privacy settings.""") + '</p>',
            ...
            ('unsubscribe_policy', mm_cfg.Radio, (_('No'), _('Yes')), 0,
             ...
             really upset if they can't get off lists!).</p>
        ...
        sender_rtn = [
            _("""<p>When a message is posted to the list, a series of
            moderation steps are take to decide whether the a moderator must
            first approve the message or not.  This section contains the
            controls for moderation of both member and non-member postings.</p>

            <p>Member postings are held for moderation if their
            <strong>moderation flag</strong> is turned on.  You can control whether
            member postings are moderated by default or not.</p>
            ...
            use a single backslash).</p>

            <p>Note that non-regexp matches are always done first.</p>"""),
            ...
            ('default_member_moderation', mm_cfg.Radio, (_('No'), _('Yes')),
             ...
             specific individual's postings will be moderated or not.</p>
            ...
            ('member_moderation_action', mm_cfg.Radio,
             ...
             list."""),
	     _('Available options') + ':</p>' +
	     '''
	     <ul>
	     	<li><strong>''' + _('Hold') + '''</strong> -- ''' + _('this holds the message for approval by the list moderators.') + '''</li>
             	<li><strong>''' + _('Reject') + '''</strong> -- ''' + _('''this automatically rejects the message by sending a bounce notice to the post's author. The text of the bounce notice can be <a href="?VARHELP=privacy/sender/member_moderation_notice">configured by you</a>.''') + '''</li>
                <li><strong>''' + _('Discard') + '''</strong> -- ''' + _('''this simply discards the message, with no notice sent to the post's author.''') + '''</li>
             </ul>
	     ...
	     '''),
            ...
            ('accept_these_nonmembers', mm_cfg.EmailListEx, (10, WIDTH), 1,
             ...
             _("""Postings from any of these non-members will be automatically accepted with no further moderation applied.""") +
	     '</p>' +
	     '<p>' +
             _('Add member addresses one per line; start the line with a ^ character to designate a regular expression match.')),

            ('hold_these_nonmembers', mm_cfg.EmailListEx, (10, WIDTH), 1,
             _("""List of non-member addresses whose postings will be
             immediately held for moderation."""),

             _("""Postings from any of these non-members will be immediately and automatically held for moderation by the list moderators. The sender will receive a notification message which will allow them to cancel their held message.""") +
	     '</p>' + 
	     '<p>' + 
             _('Add member addresses one per line; start the line with a ^ character to designate a regular expression match.')),
	     ...
	     _('Add member addresses one per line; start the line with a ^ character to designate a regular expression match.')),
            ...
            ('nonmember_rejection_notice', mm_cfg.Text, (10, WIDTH), 1,
             ...
             the list's owner address by <tt>%%(listowner)s</tt> and replaces the
	    ...
	    '</p>',
            ...
            ('require_explicit_destination', mm_cfg.Radio,
             ...
             _("""Many (in fact, most) spams do not explicitly name their myriad destinations in the explicit destination addresses - in fact often the To: field has a totally bogus address for obfuscation. The constraint applies only to the stuff in the address before the '@' sign, but still catches all such spams.""") +
	     '</p>' +
	     '<p>' +
             _('The cost is that the list will not accept unhindered any postings relayed from other addresses, unless:') + '''</p>

             <ol>
                 <li>''' + _('The relaying address has the same name, or') + '''</li>
                 <li>''' + _('The relaying address name is included on the options that specifies acceptable aliases for the list.') + '''</li>
	     ...
	     '''),
            ...
            ('acceptable_aliases', mm_cfg.Text, (4, WIDTH), 0,
             ...
             anchored to the start of the string.</p>
             
             <p>For backwards compatibility with Mailman 1.1, if the regexp
             does not contain an `@', then the pattern is matched against just
             the local part of the recipient address.  If that match fails, or
             if the pattern does contain an `@', then the pattern is matched
             against the entire recipient address.</p>
	    ...
	    '</p>', 
            ...
            ('header_filter_rules', mm_cfg.HeaderFilter, 0, 0,
             ...
             temporarily disable a rule.</p>

             <p>You can have more than one filter rule for your list.  In that
             case, each rule is matched in turn, with processing stopped after
             the first match.</p>

             <p>Note that headers are collected from all the attachments 
            ...
            ('bounce_matching_headers', mm_cfg.Text, (6, WIDTH), 0,
             ...
             _("""Use this option to prohibit posts according to specific header values. The target value is a regular-expression for matching against the specified header. The match is done disregarding letter case. Lines beginning with '#' are ignored as comments.""") + 
	     '</p>' + 
	     '<p>' + 
	     _('For example:') + 
	     '</p>' + 
	     ...
	     <p>
	     ''' +
	     _("""says to hold all postings with a <em>To:</em> mail header containing '@public.com' anywhere among the addresses.""") +
	     '</p>' + '<p>' +
             _('Note that leading whitespace is trimmed from the regexp. This can be circumvented in a number of ways, e.g. by escaping or bracketing it.')),
          ]
