...
def main():
	...
	title = 'Error'
	doc.SetTitle(title)
        doc.addMessage(_('Invalid options to CGI script'), title, 'class="message error"')
	...
	doc.SetTitle(title)
        doc.addMessage(_('The <tt>%(safelistname)s</tt> mailing list does not exist.'), 
		title, 'class="message error"')
	doc.AddItem(MailmanLogo())
        print doc.Format()
...
def process_form(mlist, doc, cgidata, lang):
    ...
    if not email:
        results.append(Paragraph(_('You must supply a valid email address.')).Format())
    ...
    if email == mlist.GetListEmail():
        ...
        results.append(Paragraph(_('You may not subscribe a list to itself!')).Format())
    ...
    if password is None and confirmed is None:
        ...
        results.append(Paragraph(_('If you supply a password, you must confirm it.')).Format())
    elif password <> confirmed:
        results.append(Paragraph(_('Your passwords did not match.')).Format())
    ...
    # web page for this cgi
        ...
        results = Paragraph(_("""The email address you supplied is banned from this mailing list.  If you think this restriction is erroneous, please contact the list owners at %(listowner)s.""")).Format()
    except Errors.MMBadEmailError:
        results = Paragraph(_("""The email address you supplied is not valid. (E.g. it must contain an `@'.)""")).Format()
    except Errors.MMHostileAddress:
        results = Paragraph(_('Your subscription is not allowed because the email address you gave is insecure.')).Format()
    except Errors.MMSubscribeNeedsConfirmation:
        # Results string depends on whether we have private rosters or not
        if privacy_results:
            results = privacy_results
        else:
            results = Paragraph(_("""Confirmation from your email address is required, to prevent anyone from subscribing you without permission. Instructions are being sent to you at %(email)s. Please note your subscription will not start until you confirm your subscription.""")).Format()
        ...
        if privacy_results:
            ...
            x = _(x)
            results = Paragraph(_("""Your subscription request was deferred because %(x)s. Your request has been forwarded to the list moderator. You will receive email informing you of the moderator's decision when they get to your request.""")).Format()
    except Errors.MMAlreadyAMember:
        # Results string depends on whether we have private rosters or not
        if not privacy_results:
            results = Paragraph(_('You are already subscribed.')).Format()
        else:
            results = privacy_results
...
"""), lang=mlang)
    ...
    # These shouldn't happen unless someone's tampering with the form
        ...
        results = Paragraph(_('This list does not support digest delivery.')).Format()
    except Errors.MMMustDigestError:
        results = Paragraph(_('This list only supports digest delivery.')).Format()
    else:
        ...
        if privacy_results:
            ...
            results = Paragarph(_("""You have been successfully subscribed to the <tt>%(realname)s</tt> mailing list.""")).Format()
    # Show the results
