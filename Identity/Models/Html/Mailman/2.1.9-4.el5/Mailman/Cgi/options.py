...
def main():
    ...
    try:
        ...
        title = _('CGI script error')
        doc.SetTitle(title)
        doc.addError(_('No such list <em>%(safelistname)s</em>'), title)
        doc.AddItem(MailmanLogo())
        print doc.Format()
        ...
        if mlist.isMember(user):
            ...
            try:
                ...
                if mlist.unsubscribe_policy:
                    ...
                    doc.addMessage(_('Your unsubscription request has been forwarded to the list administrator for approval.'),
                                 tag='', css='class="message warning strong"')
                else:
                    mlist.ConfirmUnsubscription(user, userlang)
                    doc.addMessage(_('The confirmation email has been sent.'),
                                 tag='', css='class="message warning strong"')
                mlist.Save()
            finally:
                mlist.Unlock()
            ...
            if mlist.private_roster == 0:
                ...
                syslog('mischief',
                       _('Unsub attempt of non-member w/ private rosters: %s'),
                       user)
                doc.addMessage(_('The confirmation email has been sent.'),
                                 tag='', css='class="message success strong"')
    ...
    if cgidata.has_key('emailpw'):
        mlist.MailUserPassword(user)
	msg = Div(Paragraph(_('A reminder of your password has been emailed to you.'))).Format(css='class="message success strong"')
    ...
    if cgidata.has_key('othersubs'):
	    ...
	    msg = Div(Paragraph(_('The list administrator may not view the other subscriptions for this user.'))).Format(css='class="message error strong"')
            options_page(mlist, doc, user, cpuser, userlang, msg)
            print doc.Format()
            return
        hostname = mlist.host_name
        title = _('List of subscriptions for: %(safeuser)s')
        doc.SetTitle(title)
        doc.AddItem(Header(1, title))
        doc.AddItem(Paragraph(_('Click on a link to visit your options page for the requested mailing list.')))
	...
	msg = ''
        ...
        if globally and not is_user_or_siteadmin:
            msg += Div(Paragraph(_("The list administrator may not change the names or addresses for this user's other subscriptions. However, the subscription for this mailing list has been changed."))).Format(css='class="message warning strong"')
            globally = False
        ...
        if newaddr and confirmaddr:
	    	...
	    	msg += Div(Paragraph(_('Addresses did not match!'))).Format(css='class="message error strong"')
                options_page(mlist, doc, user, cpuser, userlang, msg)
                print doc.Format()
                return
            if newaddr == cpuser:
	    	msg += Div(Paragraph(_('You are already using that email address'))).Format(css='class="message error strong"')
                options_page(mlist, doc, user, cpuser, userlang, msg)
                print doc.Format()
                return
            ...
            if mlist.isMember(newaddr):
                ...
                if globally:
                    ...
                    msg += Div(Paragraph(_('The new address you requested %(newaddr)s is already a member of the %(listname)s mailing list, however you have also requested a global change of address. Upon confirmation, any other mailing list containing the address %(safeuser)s will be changed.'))).Format(css='class="message warning strong"')
                    # Don't return
                else:
		    msg += Div(Paragraph(
	    ...
	    msg += Div(Paragraph(_('Addresses may not be blank'))).Format(css='class="message error strong"')
            options_page(mlist, doc, user, cpuser, userlang, msg)
            print doc.Format()
            return
        ...
        if set_address:
            ...
            msg = Div(Paragraph(_('A confirmation message has been sent to: %(newaddr)s.'))).Format(css='class="message success strong"')
            mlist.Lock()
            try:
                try:
                ...
                msg = Div(Paragraph(_('Bad email address provided'))).Format(css='class="message error strong"')
            except Errors.MMHostileAddress:
                msg = Div(Paragraph(_('Illegal email address provided'))).Format(css='class="message error strong"')
            except Errors.MMAlreadyAMember:
                msg = Div(Paragraph(_('%(newaddr)s is already a member of the list.'))).Format(css='class="message error strong"')
            except Errors.MembershipIsBanned:
                owneraddr = mlist.GetOwnerEmail()
                msg = Div(Paragraph(_('%(newaddr)s is banned from this list. If you think this restriction is erroneous, please contact the list owners at %(owneraddr)s.'))).Format(css='class="message warning strong"')
        ...
        if set_membername:
            ...
            try:
                ...
                mlist.Unlock()
            msg += Div(Paragraph(_('Member name successfully changed.'))).Format(css='class="message success strong"')
    ...
    if cgidata.has_key('changepw'):
        ...
        if not newpw or not confirmpw:
            options_page(mlist, doc, user, cpuser, userlang,
                         Div(Paragraph(_('Passwords may not be blank'))).Format(css='class="message error strong"'))
            print doc.Format()
            return
        if newpw <> confirmpw:
            options_page(mlist, doc, user, cpuser, userlang,
                         Div(Paragraph(_('Passwords did not match!'))).Format(css='class="message error strong"'))
            print doc.Format()
            return
	...
	msg = ''
        ...
        if pw_globally and not is_user_or_siteadmin:
            msg += Div(Paragraph(_('''The list administrator may not change the password for this user's other subscriptions. However, the password for this mailing list has been changed.'''))).Format(css='class="message warning strong"')
            pw_globally = False
	...
	msg += Div(Paragraph(_('Password successfully changed.'))).Format(css='class="message success strong"')
        options_page(mlist, doc, user, cpuser, userlang, msg)
        print doc.Format()
        return
    ...
    if cgidata.has_key('unsub'):
        ...
        if not cgidata.getvalue('unsubconfirm'):
            options_page(
                ...
                Div(Paragraph(_('''You must confirm your unsubscription request by turning on the checkbox below the <em>Unsubscribe</em> button. You have not been unsubscribed!'''))).Format(css='class="message error strong"'))
            print doc.Format()
            return
        ...
        doc.AddItem(Header(1, title))
        if needapproval:
            doc.AddItem(Paragraph(_('Your unsubscription request has been received and forwarded on to the list moderators for approval. You will receive notification once the list moderators have made their decision.')))
        else:
	    doc.AddItem(Paragraph(_('You have been successfully unsubscribed from the mailing list %(fqdn_listname)s. If you were receiving digest deliveries you may get one more digest. If you have any questions about your unsubscription, please contact the list owners at %(owneraddr)s.')))
    ...
    if cgidata.has_key('options-submit'):
        ...
        if globalopts:
            if not is_user_or_siteadmin:
                doc.addMessage(_("""The list administrator may not change the options for this user's other subscriptions. However the options for this mailing list subscription has been changed."""), css='class="message warning strong"')
            else:
        ...
        if cantdigest:
            msg = _('''The list administrator has disabled digest delivery for this list, so your delivery option has not been set. However your other options have been set successfully.''')
        elif mustdigest:
            msg = _('''The list administrator has disabled non-digest delivery for this list, so your delivery option has not been set. However your other options have been set successfully.''')
        else:
	...
	# Set message presentation
        if cantdigest or mustdigest or digestwarn:
	    msg = Div(Paragraph(msg)).Format(css='class="message warning strong"')
	else:
	    msg = Div(Paragraph(msg)).Format(css='class="message success strong"')
...
def options_page(mlist, doc, user, cpuser, userlang, message=''):
    ...
    if message:
	# message should come formatted with 
	# Div(Paragraph()).Format(css='class="message success|error"')
    ...
    replacements['<mm-unsubscribe-button>'] = (
        mlist.FormatButton('unsub', _('Unsubscribe')) + '<p>' +
        CheckBox('unsubconfirm', 1, checked=0).Format() +
        _('<em>Yes, I really want to unsubscribe</em>') + '</p>')
    ...
    if cpuser is not None:
        replacements['<mm-case-preserved-user>'] = Div(
		Paragraph(_('''You are subscribed to this list with the case-preserved address: %(cpuser)s.''')
		)).Format(css='class="message warning strong"')
    else:
        replacements['<mm-case-preserved-user>'] = ''

    # Links on header section (errormsg)
    # Beacause this is on a html template file replacements is need in
    # order to make the exapantion of these links.
...
def loginpage(mlist, doc, user, lang):
    ...
    # Set up the title.
    title = _('%(realname)s: membership options')
    doc.SetTitle(title)
    doc.AddItem(Header(1, title))

    # Set up what this page is for
    if user is None:
        title = _('Login page')
        extra = _('email address and ')
    else:
        safeuser = Utils.websafe(user)
        title = _('for user: ') + safeuser
        obuser = Utils.ObscureEmail(user)
        extra = ''

    doc.AddItem(Paragraph(title).Format(0, 'class="strong"'))
    ...
    if len(mlist.GetAvailableLanguages()) > 1:
        ...
        langform.AddItem(Paragraph(SubmitButton('displang-button',
                                      _('View this page in')).Format(), 
				   mlist.GetLangSelectBox(lang)).Format())
        if user:
            langform.AddItem(Paragraph(Hidden('email', user)))
    ...
    form = Form(actionurl)
    table = Table()
    doc.AddItem(Paragraph(_("""In order to change your membership option, you must first log in by giving your %(extra)s membership password in the section below. If you don't remember your membership password, you can have it emailed to you by clicking on the button below. If you just want to unsubscribe from this list, click on the <em>Unsubscribe</em> button and a confirmation message will be sent to you.""")))

    doc.AddItem(Paragraph(_('''<strong>Important:</strong> From this point on, you must have cookies enabled in your browser, otherwise none of your changes will take effect.''')))

    # Password and login button
    table.AddRow([Header(3, _('Login'))])
    table.AddCellInfo(table.GetCurrentRowIndex(), 0, colspan=2, css='class="center"')
    if user is None:
        table.AddRow([_('Email address:'),
        ...
        table.AddRow([Hidden('email', user)])
    table.AddRow([_('Password'), PasswordBox('password', size=20)]) 
    table.AddCellInfo(table.GetCurrentRowIndex(), 0, css='class="description"')
    table.AddCellInfo(table.GetCurrentRowIndex(), 1, css='class="value"')

    table.AddRow(['', SubmitButton('login', _('Log in'))])
    table.AddCellInfo(table.GetCurrentRowIndex(), 0, css='class="description"')
    table.AddCellInfo(table.GetCurrentRowIndex(), 1, css='class="value"')

    # Unsubscribe section
    table.AddRow([Header(3, _('Unsubscribe'))])
    table.AddCellInfo(table.GetCurrentRowIndex(), 0, colspan=2, css='class="center"')

    table.AddRow([_("""By clicking on the <em>Unsubscribe</em> button, a confirmation message will be emailed to you. This message will have a link that you should click on to complete the removal process (you can also confirm by email; see the instructions in the confirmation message)."""),
    		SubmitButton('login-unsub', _('Unsubscribe'))])
    table.AddCellInfo(table.GetCurrentRowIndex(), 0, css='class="description"')
    table.AddCellInfo(table.GetCurrentRowIndex(), 1, css='class="value"')

    # Password reminder section
    table.AddRow([Header(3, _('Password reminder'))])
    table.AddCellInfo(table.GetCurrentRowIndex(), 0, colspan=2, css='class="center"')

    table.AddRow([_("""By clicking on the <em>Remind</em> button, your password will be emailed to you."""), 
    		   SubmitButton('login-remind', _('Remind'))])
    table.AddCellInfo(table.GetCurrentRowIndex(), 0, css='class="description"')
    table.AddCellInfo(table.GetCurrentRowIndex(), 1, css='class="value"')
...
def topic_details(mlist, doc, user, cpuser, userlang, varhelp):
    ...
    table = Table()
    table.AddRow([Header(3,_('Topic filter details'))])
    table.AddCellInfo(table.GetCurrentRowIndex(), 0, colspan=2, css='class="center"')
    table.AddRow([_('Name:'),
                  Utils.websafe(name)])
    table.AddCellInfo(table.GetCurrentRowIndex(), 0, css='class="description"')
    table.AddCellInfo(table.GetCurrentRowIndex(), 1, css='class="value"')
    table.AddRow([_('Pattern (as regexp):'),
                  '<tt>' + Utils.websafe(pattern) + '</tt>'])
    table.AddCellInfo(table.GetCurrentRowIndex(), 0, css='class="description"')
    table.AddCellInfo(table.GetCurrentRowIndex(), 1, css='class="value"')
