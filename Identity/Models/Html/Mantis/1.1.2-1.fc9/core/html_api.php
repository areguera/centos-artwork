...
# along with Mantis.  If not, see <http://www.gnu.org/licenses/>.
	...
	function html_top_banner() {
		...
		if ( !is_blank( $t_page ) && file_exists( $t_page ) && !is_dir( $t_page ) ) {
			...
			PRINT '<div class="pageline"><hr style="display:none;"></div>';
			print_menu();
			...
			if ( auth_is_user_authenticated() ) {
			print '<div class="recently_visited">';
			print_recently_visited();
			print '</div>';
			print_projectinfo();	

			PRINT '<div class="pageline2"><hr style="display:none;"></div>';
	...
	function html_footer( $p_file ) {
		...
		print '</div>'; # close page div. Opened at html_top_banner.
		...
		echo '<div class="poweredby"><a href="http://www.mantisbt.org" title="Free Web Based Bug Tracker"><img src="images/mantis_logo_button.gif" width="88" height="35" alt="Powered by Mantis Bugtracker" border="0" /></a></div>';

		print '<div id="credits">';

		# print Mantis version
		if ( ON == config_get( 'show_version' ) ) {
			echo "\t", '<a href="http://www.mantisbt.org/" title="Free Web Based Bug Tracker">Mantis ', MANTIS_VERSION, '</a>', '[<a href="http://www.mantisbt.org/"  title="Free Web Based Bug Tracker" target="_blank">^</a>]<br />', "\n";
		}
		...
		if ( ! is_page_name( 'login_page.php' ) && !current_user_is_anonymous() ) {
			echo "\t", '<a href="mailto:', config_get( 'webmaster_email' ), '">', config_get( 'webmaster_email' ), '</a><br />', "\n";
		}
		...
		if ( helper_show_queries() ) {
			...
			if ( ON == config_get( 'show_queries_list' ) ) {
				echo "\t",  '<table style="background:#fff;">', "\n";
				$t_total = 0;
				for ( $i = 0; $i < $t_count; $i++ ) {
	...
	function print_menu() {
		if ( auth_is_user_authenticated() ) {
			...
			PRINT '<ul class="navibar">';
				$t_menu_options = array();

				# Main Page
				$t_menu_options[] = '<li class="first"><a href="main_page.php">' . lang_get( 'main_link' ) . '</a></li>';

				# My View
				$t_menu_options[] = '<li><a href="my_view_page.php">' . lang_get( 'my_view_link' ) . '</a></li>';

				# View Bugs
				$t_menu_options[] = '<li><a href="view_all_bug_page.php">' . lang_get( 'view_bugs_link' ) . '</a></li>';
				...
				if ( access_has_project_level( config_get( 'view_changelog_threshold' ) ) ) {
					$t_menu_options[] = '<li><a href="changelog_page.php">' . lang_get( 'changelog_link' ) . '</a></li>';
				}

				# Roadmap Page
				if ( access_has_project_level( config_get( 'roadmap_view_threshold' ) ) ) {
					$t_menu_options[] = '<li><a href="roadmap_page.php">' . lang_get( 'roadmap_link' ) . '</a></li>';
				}

				# Summary Page
				if ( access_has_project_level( config_get( 'view_summary_threshold' ) ) ) {
					$t_menu_options[] = '<li><a href="summary_page.php">' . lang_get( 'summary_link' ) . '</a></li>';
				}

				# Project Documentation Page
				if( ON == config_get( 'enable_project_documentation' ) ) {
					$t_menu_options[] = '<li><a href="proj_doc_page.php">' . lang_get( 'docs_link' ) . '</a></li>';
				}

				# Project Wiki
				if ( wiki_is_enabled() ) {
					$t_menu_options[] = '<li><a href="wiki.php?type=project&amp;id=' . $t_current_project . '">' . lang_get( 'wiki' ) . '</a></li>';
				}
				...
				if ( access_has_global_level( $t_show_access) || access_has_any_project( $t_show_access ) )  {
					...
					$t_menu_options[] = "<li><a href=\"$t_link\">" . lang_get( 'manage_link' ) . '</a></li>';
				}
				...
				if ( access_has_project_level( config_get( 'manage_news_threshold' ) ) ) {
					...
					if ( ( ALL_PROJECTS != helper_get_current_project() ) || ( access_has_project_level( ADMINISTRATOR ) ) ) {
						$t_menu_options[] = '<li><a href="news_menu_page.php">' . lang_get( 'edit_news_link' ) . '</a></li>';
					} else {
						$t_menu_options[] = '<li><a href="login_select_proj_page.php">' . lang_get( 'edit_news_link' ) . '</a></li>';
					}
				}

				# Account Page (only show accounts that are NOT protected)
				if ( OFF == $t_protected ) {
					$t_menu_options[] = '<li><a href="account_page.php">' . lang_get( 'account_link' ) . '</a></li>';
				}
				...
				if ( config_get('time_tracking_enabled') && config_get('time_tracking_with_billing') )
					$t_menu_options[] = '<li><a href="billing_page.php">' . lang_get( 'time_tracking_billing_link' ) . '</a></li>';
