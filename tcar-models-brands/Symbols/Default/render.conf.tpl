# This file contains the information used to build The CentOS Logo in
# different sizes, colors and formats.

[centos.png]
render-type = "svg"
render-from = "${TCAR_BASEDIR}/Artworks/Brands/Symbols/Default/centos.svgz"
render-flow = "extended"
formats     = "xpm pdf jpg tif"
heights     = "11 16 20 22 24 26 32 36 38 40 48 52 64 72 78 96 112 124 128 148 164 196 200 512"
fgcolors    = "000000 ffffff"
bgcolors    = "ffffff-0"
