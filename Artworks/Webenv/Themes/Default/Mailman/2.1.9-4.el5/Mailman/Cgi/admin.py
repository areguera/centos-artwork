...
def main():
    ...
    try:
        ...
        if not mlist.digestable and not mlist.nondigestable:
            doc.addError(
                _('''You have turned off delivery of both digest and non-digest messages. This is an incompatible state of affairs. You must turn on either digest delivery or non-digest delivery or your mailing list will basically be unusable.'''), tag=_('Warning: '))

        if not mlist.digestable and mlist.getDigestMemberKeys():
            doc.addError(
                _('''You have digest members, but digests are turned off. Those people will not receive mail.'''),
                tag=_('Warning: '))
        if not mlist.nondigestable and mlist.getRegularMemberKeys():
            doc.addError(
                _('''You have regular list members but non-digestified mail is turned off.  They will receive mail until you fix this problem.'''), tag=_('Warning: '))
...
def admin_overview(msg=''):
    ...
    title = _('Administration') + ' - ' + _('Mailing Lists')
    # The html `document'
    doc = Document()
    ...
    welcome = Header(1, _('Administration')).Format()
    mailmanlink = Link(mm_cfg.MAILMAN_URL, _('Mailman')).Format()
    if not advertised:
        welcome += Paragraph(
            _('''There currently are no publicly-advertised %(mailmanlink)s mailing lists on %(hostname)s.''')).Format(css='class="strong"')
    else:
        welcome += Paragraph(
            _('''Below is the collection of publicly-advertised %(mailmanlink)s mailing lists on %(hostname)s. Click on a list name to visit the configuration pages for that list.''')).Format()

    creatorurl = Utils.ScriptURL('create')
    mailman_owner = Utils.get_site_email()
    extra = msg and _('right ') or ''
    welcome += Paragraph(
        _('''To visit the administrators configuration page for an unadvertised list, open a URL similar to this one, but with a '/' and the %(extra)slist name appended. If you have the proper authority, you can also <a href="%(creatorurl)s">create a new mailing list</a>.''')).Format()

    welcome += Paragraph(
    	_('''General list information can be found at ''') +
        Link(Utils.ScriptURL('listinfo'),
             _('the mailing list overview page')).Format() + '.').Format()

    welcome += Paragraph(_('(Send questions and comments to ') +
        Link('mailto:%s' % mailman_owner, mailman_owner).Format() +
        '.)').Format()
...
def option_help(mlist, varhelp):
    ...
    doc.AddItem(Header(1, _('%(realname)s Mailing list Configuration Help')).Format())
    doc.AddItem(Header(2, _('%(varname)s Option')).Format())
    doc.AddItem(Paragraph(_("<strong>%s</strong> (%s): %s" % (varname, category, description))).Format())

    if elaboration:
        doc.AddItem(Paragraph("%s" % elaboration).Format())
	...
	_('''<strong>Warning:</strong> changing this option here could cause other screens to be out-of-sync.  Be sure to reload any other pages that are displaying this option for this mailing list. You can also ''') 
        + Link(url, _('return to the %(categoryname)s options page')).Format()
        + '.'))
...
def show_results(mlist, doc, category, subcat, cgidata):
    ...
    doc.AddItem(Header(1, _('%(realname)s mailing list administration')).Format())
    ...
    # ...and add the links to the document.
    form.AddItem('<div class="adminpanel col2">')
    form.AddItem('<p class="adminpanel">' + _('Other options') + '</p>')
    form.AddItem(otherlinks.Format(css='class="adminpanel"'))
    form.AddItem('</div>')
    form.AddItem('<div class="adminpanel col1">')
    form.AddItem('<p class="adminpanel">' + _('Configuration options') + '</p>')
    form.AddItem(categorylinks_1.Format(css='class="adminpanel floatl"'))
    form.AddItem(categorylinks_2.Format(css='class="adminpanel floatl"'))
    form.AddItem('<hr class="transparent" />')
    # Make the emergency stop switch a rude solo light
    if mlist.emergency:
        label = _('Emergency moderation of all list traffic is enabled')
        form.AddItem(Paragraph(Link('?VARHELP=general/emergency', label)).Format(css='class="emergency"'))
    form.AddItem('</div>')
    form.AddItem(Paragraph(_('''Make your changes in the following section, then submit them using the <em>Submit Your Changes</em> button below.''')).Format())
    ...
    if category == 'members':
        ...
        form.AddItem(submit_button('setmemberopts_btn'))

        # In "list" subcategory, we can also search for members
        if subcat == 'list':
            table = Table()
            container = Container()
            container.AddItem(Header(2, _('Additional Member Tasks')).Format())

            # Add a section to set the moderation bit for all members
            table.AddRow([
		_("""Set everyone's moderation bit, including those members not currently visible:"""),
			       RadioButtonArray('allmodbit_val',
                                           (_('Off'), _('On')),
	    ...
	    table.AddRow([SubmitButton('allmodbit_btn', _('Set'))]) 
        ...
        form.AddItem(password_inputs(mlist))
        form.AddItem(submit_button())
    else:
...
def show_variables(mlist, category, subcat, cgidata, doc):
    ...
    container = Container()
    container.AddItem(Header(2, label).Format())
    ...
    table.AddRow([_('Description:'),
                  _('Value')])
    table.AddCellInfo(max(table.GetCurrentRowIndex(), 0), 0, css='class="description center strong"')
    table.AddCellInfo(max(table.GetCurrentRowIndex(), 0), 1, css='class="value center strong"')
    ...
    for item in options:
        if type(item) == StringType:
            ...
            table.AddRow([Header(3, item).Format(css='class="center"')])
...
def get_item_characteristics(record):
    ...
    if len(record) == 5:
        ...
        raise ValueError, _('Badly formed options entry: %(record)s')
...
def get_item_gui_value(mlist, category, kind, varname, params, extra):
    ...
    if kind == mm_cfg.Radio or kind == mm_cfg.Toggle:
        ...
        if params:
            ...
            r, c = None, None
        res = NL.join(value)
        return TextArea(varname, res, r, c)
        ...
        if params:
            ...
            r, c = None, None
        container = Container()
        container.AddItem('<em>' + _('Enter the text below') + ':</em><br />')
        container.AddItem(TextArea(varname, value or '', r, c))
        container.AddItem('<br /><em>' + _('... or specify a file to upload') + ':</em><br />')
        ...
        def makebox(i, name, pattern, desc, empty=False, table=table):
            ...
            if empty:
                table.AddRow([Paragraph(_('Topic %(i)d')).Format(),
                              Paragraph(Hidden(newtag)).Format()])
	 	table.AddCellInfo(table.GetCurrentRowIndex(), 0, css='class="header strong center"')
	 	table.AddCellInfo(table.GetCurrentRowIndex(), 1, css='class="header"')
            else:
                table.AddRow([Paragraph(_('Topic %(i)d')).Format(),
                              Paragraph(SubmitButton(deltag, _('Delete'))).Format()])
	 	table.AddCellInfo(table.GetCurrentRowIndex(), 0, css='class="header strong center"')
	 	table.AddCellInfo(table.GetCurrentRowIndex(), 1, css='class="header"')

            table.AddRow([_('Topic name:'),
                          TextBox(boxtag, value=name, size=30)])
	    table.AddCellInfo(table.GetCurrentRowIndex(), 0, css='class="right"')
	    table.AddCellInfo(table.GetCurrentRowIndex(), 1, css='class="left"')

            table.AddRow([_('Regexp:'),
        ...
        def makebox(i, pattern, action, empty=False, table=table):
            ...
            if empty:
                table.AddRow([Paragraph(_('Spam Filter Rule %(i)d')).Format(),
                              Paragraph(Hidden(newtag)).Format()])
	 	table.AddCellInfo(table.GetCurrentRowIndex(), 0, css='class="header strong center"')
	 	table.AddCellInfo(table.GetCurrentRowIndex(), 1, css='class="header"')
            else:
	 	...
	 	table.AddCellInfo(table.GetCurrentRowIndex(), 1, css='class="header"')
            ...
            table.AddRow([_('Spam Filter Regexp:'),
                          TextArea(reboxtag, text=pattern,
                                   rows=4, cols=30)])
	    table.AddCellInfo(table.GetCurrentRowIndex(), 0, css='class="right"')
	    table.AddCellInfo(table.GetCurrentRowIndex(), 1, css='class="left"')
...
def get_item_gui_description(mlist, category, subcat,
    ...
    if detailsp:
        ...
        if descr == elaboration:
            linktext = _('(Edit <strong>%(varname)s</strong>)')
        else:
            linktext = _('(Details for <strong>%(varname)s</strong>)')
        link = Link(mlist.GetScriptURL('admin') + varhelp,
                    linktext).Format()
        text = '%s<br />%s' % (descr, link)
    else:
        text = descr
    if varname[0] == '_':
        text += Paragraph(_('''<strong>Note:</strong> setting this value performs an immediate action but does not modify permanent state.''')).Format()
    return text
...
def membership_options(mlist, subcat, cgidata, doc, form):
    ...
    if subcat == 'add':
        container.AddItem(Header(2, _('Mass Subscriptions')).Format())
        mass_subscribe(mlist, container)
        return container
    if subcat == 'remove':
        container.AddItem(Header(2, _('Mass Removals')).Format())
        mass_remove(mlist, container)
        return container
    # Otherwise...
    container.AddItem(Header(2, _('Membership List')).Format())
    # Add a "search for member" button
    link = Link('http://www.python.org/doc/current/lib/re-syntax.html',
                _('(help)')).Format()
    container.AddItem(Paragraph(
    	_('Find member %(link)s:') +
	TextBox('findmember', value=cgidata.getvalue('findmember', '')).Format() +
	SubmitButton('findmember_btn', _('Search...')).Format()).Format())

    usertable = Table(css='class="mm_usertable"')
    ...
    if bucket:
        ...
        usertable.AddRow([Header(3, 
		_('%(allcnt)s members total, %(membercnt)s shown')).Format()])
    else:
        usertable.AddRow([Header(3, 
		_('%(allcnt)s members total')).Format()])
    ...
    if bucket:
        ...
        joiner = '&nbsp;'*2 + '\n'
        usertable.AddRow([joiner.join(cells)])
        usertable.AddCellInfo(usertable.GetCurrentRowIndex(),
                          usertable.GetCurrentCellIndex(),
                          colspan=OPTCOLUMNS,
			  css='class="center"')

    usertable.AddRow([(h) for h in (_('unsub'),
    ...
    for i in range(OPTCOLUMNS):
        usertable.AddCellInfo(rowindex, i, css='class="header strong center"')
    ...
    for addr in members:
        ...
        cells = [CheckBox(addr + '_unsub', 'off', 0).Format(),
                 link.Format() + '<br />' +
                 name +
        ...
        if addr in mlist.getRegularMemberKeys():
            cells.append(CheckBox(addr + '_digest', 'off', 0).Format())
        else:
            cells.append(CheckBox(addr + '_digest', 'on', 1).Format())
        ...
        try:
            ...
            selected = 0
        cells.append(SelectOptions(addr + '_language', langs,
                                          langdescs, selected).Format())

	# Put each cell into the row and give format to that row
        usertable.AddRow(cells)
	if highlight:
		usertable.AddRowInfo(usertable.GetCurrentRowIndex(), css='class="title center"')
		highlight = not highlight
	else:
		highlight = 1
		usertable.AddRowInfo(usertable.GetCurrentRowIndex(), css='class="center"')

    # Add the usertable and a legend
    legend = UnorderedList()
    legend.AddItem(
        _('<strong>unsub</strong> -- Click on this to unsubscribe the member.'))
    legend.AddItem(
        _("""<strong>mod</strong> -- The user's personal moderation flag.  If this is
        set, postings from them will be moderated, otherwise they will be
        approved."""))
    legend.AddItem(
        _("""<strong>hide</strong> -- Is the member's address concealed on
        the list of subscribers?"""))
    legend.AddItem(
    	_('<strong>nomail</strong> -- Is delivery to the member disabled?  If so, an abbreviation will be given describing the reason for the disabled delivery:') + '\n' +
        '    <ul>' + '\n' +
	_('''    <li><strong>U</strong> -- Delivery was disabled by the user via their personal options page.</li>''') + '\n' +
        _('''    <li><strong>A</strong> -- Delivery was disabled by the list administrators.</li>''') + '\n' +
        _('''    <li><strong>B</strong> -- Delivery was disabled by the system due to excessive bouncing from the member's address.</li>''') + '\n' +
        _('''    <li><strong>?</strong> -- The reason for disabled delivery isn't known. This is the case for all memberships which were disabled in older versions of Mailman.</li>''') + '\n' +
        '    </ul>' + '\n')
    legend.AddItem(
        _('''<strong>ack</strong> -- Does the member get acknowledgements of their
        posts?'''))
    legend.AddItem(
        _('''<strong>not metoo</strong> -- Does the member want to avoid copies of their
        own postings?'''))
    legend.AddItem(
        _('''<strong>nodupes</strong> -- Does the member want to avoid duplicates of the
        same message?'''))
    legend.AddItem(
        _('''<strong>digest</strong> -- Does the member get messages in digests?
        (otherwise, individual messages)'''))
    legend.AddItem(
        _('''<strong>plain</strong> -- If getting digests, does the member get plain
        text digests?  (otherwise, MIME)'''))
    legend.AddItem(_("<strong>language</strong> -- Language preferred by the user"))
    addlegend = ''
    parsedqs = 0
    qsenviron = os.environ.get('QUERY_STRING')
    if qsenviron:
    ...
    container.AddItem(usertable)

    # There may be additional chunks
    if chunkindex is not None:
        buttons = []
        url = adminurl + '/members?%sletter=%s&' % (addlegend, bucket)
        footer = _('''To view more members, click on the appropriate range listed below:''')
        ...
        container.AddItem(Paragraph(footer + buttons.Format()).Format())
    return container
...
def mass_subscribe(mlist, container):
    ...
    table.AddRow([
        _('Send welcome messages to new subscribees?'),
    ...
    table.AddRow([
        _('Send notifications of new subscriptions to the list owner?'),
    ...
    table.AddRow([
		  _('Enter one address per line:'),
    		  TextArea(name='subscribees',
                                  rows=10, cols='70%').Format() +
    		  Paragraph(_('... or specify a file to upload:') +
                  FileUpload('subscribees_upload', cols='50').Format()).Format()
		  ])
    table.AddCellInfo(table.GetCurrentRowIndex(), 0, css='class="description"')
    table.AddCellInfo(table.GetCurrentRowIndex(), 1, css='class="value"')

    # Invitation text
    table.AddRow([_("""Enter additional text to be added to the
    top of your invitation or the subscription notification.  Include at least
    one blank line at the end:"""),
		TextArea(name='invitation', rows=10, cols='70%')])
    table.AddCellInfo(table.GetCurrentRowIndex(), 0, css='class="description"')
    table.AddCellInfo(table.GetCurrentRowIndex(), 1, css='class="value"')
...
def mass_remove(mlist, container):
    ...
    table = Table()
    table.AddRow([
        _('Send unsubscription acknowledgement to the user?'),
    ...
    table.AddRow([
        _('Send notifications to the list owner?'),
    ...
    table.AddRow([_('Enter one address per line:'),
    		  TextArea(name='unsubscribees',
                                  rows=10, cols='70%').Format() 
		+ Paragraph(_('... or specify a file to upload:')
		+ FileUpload('unsubscribees_upload', cols='50').Format()).Format()
		])
    table.AddCellInfo(table.GetCurrentRowIndex(), 0, css='class="description"')
    table.AddCellInfo(table.GetCurrentRowIndex(), 1, css='class="value"')
...
def password_inputs(mlist):
    adminurl = mlist.GetScriptURL('admin', absolute=1)
    container = Container()
    container.AddItem(Header(2, _('Change list ownership passwords')).Format())
    container.AddItem(Paragraph(_('''The <em>list administrators</em> are the people who have ultimate control over all parameters of this mailing list.  They are able to change any list configuration variable available through these administration web pages.''')))
    container.AddItem(Paragraph(_('''The <em>list moderators</em> have more limited permissions; they are not able to change any list configuration variable, but they are allowed to tend to pending administration requests, including approving or rejecting held subscription requests, and disposing of held postings.  Of course, the <em>list administrators</em> can also tend to pending requests.''')))
    container.AddItem(Paragraph(_('''In order to split the list ownership duties into administrators and moderators, you must set a separate moderator password in the fields below, and also provide the email addresses of the list moderators in the <a href="%(adminurl)s/general">general options section</a>.''')))
    # Set up the admin password table on the left
    atable = Table()
    atable.AddRow([_('Enter new administrator password:'),
                   PasswordBox('newpw', size=20)])
    atable.AddCellInfo(atable.GetCurrentRowIndex(), 0, css='class="description"')
    atable.AddCellInfo(atable.GetCurrentRowIndex(), 1, css='class="value"')
    atable.AddRow([_('Confirm administrator password:'),
                   PasswordBox('confirmpw', size=20)])
    atable.AddCellInfo(atable.GetCurrentRowIndex(), 0, css='class="description"')
    atable.AddCellInfo(atable.GetCurrentRowIndex(), 1, css='class="value"')
    # Set up the moderator password table on the right
    mtable = Table()
    mtable.AddRow([_('Enter new moderator password:'),
                   PasswordBox('newmodpw', size=20)])
    mtable.AddCellInfo(mtable.GetCurrentRowIndex(), 0, css='class="description"')
    mtable.AddCellInfo(mtable.GetCurrentRowIndex(), 1, css='class="value"')
    mtable.AddRow([_('Confirm moderator password:'),
                   PasswordBox('confirmmodpw', size=20)])
    mtable.AddCellInfo(mtable.GetCurrentRowIndex(), 0, css='class="description"')
    mtable.AddCellInfo(mtable.GetCurrentRowIndex(), 1, css='class="value"')

    # Add these tables to the overall password container
    container.AddItem(atable)
    container.AddItem(mtable)
    return container
...
def submit_button(name='submit'):
    container = Container()
    container.AddItem(Paragraph(SubmitButton(name, _('Submit Your Changes'))).Format(css='class="mm_submit"'))
    return container
...
def change_options(mlist, category, subcat, cgidata, doc):
    ...
    if new or confirm:
        if new == confirm:
            ...
            doc.addMessage(_('Moderator passwords did not match'), css='class="message error strong"')
    ...
    if new or confirm:
        if new == confirm:
            ...
            doc.addMessage(_('Administrator passwords did not match'), css='class="message error strong"')
    ...
    if subscribers:
        ...
        if subscribe_success:
            if subscribe_or_invite:
                msg = _('Successfully invited:')
            else:
                msg = _('Successfully subscribed:')
	    doc.AddItem(Div(Header(3, msg).Format() 
	    	+ UnorderedList(*subscribe_success).Format(css='class="mailaddresses"')
		).Format(css='class="message success"'))

        if subscribe_errors:
            if subscribe_or_invite:
                msg = _('Error inviting:')
            else:
                msg = _('Error subscribing:')
    ...
    if removals:
        ...
        if unsubscribe_success:
            msg = _('Successfully Unsubscribed:')
	    doc.AddItem(Div(Header(3, msg).Format() 
	    	+ UnorderedList(*unsubscribe_success).Format(css='class="mailaddresses"')
		).Format(css='class="message success"'))

        if unsubscribe_errors:
            msg = _('Cannot unsubscribe non-members:')
	    doc.AddItem(Div(Header(3, msg).Format() 
	    	+ UnorderedList(*unsubscribe_errors).Format(css='class="mailaddresses"')
		).Format(css='class="message error"'))
    ...
    if cgidata.has_key('setmemberopts_btn') and cgidata.has_key('user'):
        ...
        for user in users:
            ...
            if not mlist.isMember(user):
                doc.addMessage(_('Ignoring changes to deleted member: %(user)s'),
                             tag=_('Warning: '),
			     css='class="message warning"')
                continue
