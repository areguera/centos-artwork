...
class ContentFilter(GUIBase):
    ...
    def GetConfigInfo(self, mlist, category, subcat=None):
	    ...
	    '</p>' +
	    '<p>' +
            _('Content filtering works like this: when a message is received by the list and you have enabled content filtering, the individual attachments are first compared to the <a href="?VARHELP=contentfilter/filter_mime_types">filter types</a>. If the attachment type matches an entry in the filter types, it is discarded.') +
	    '</p>' +
	    '<p>' +
            _('Then, if there are <a href="?VARHELP=contentfilter/pass_mime_types">pass types</a> defined, any attachment type that does <em>not</em> match a pass type is also discarded. If there are no pass types defined, this check is skipped.') +
	    '</p>' +
	    '<p>' +
            _('After this initial filtering, any <tt>multipart</tt> attachments that are empty are removed. If the outer message is left empty after this filtering, then the whole message is discarded.') +
	    '</p>' +
	    '<p>' +
            _('Then, each <tt>multipart/alternative</tt> section will be replaced by just the first alternative that is non-empty after filtering if <a href="?VARHELP=contentfilter/collapse_alternatives">collapse_alternatives</a> is enabled.') +
	    '</p>' +
	    '<p>' +
            _('Finally, any <tt>text/html</tt> parts that are left in the message may be converted to <tt>text/plain</tt> if <a href="?VARHELP=contentfilter/convert_html_to_plaintext">convert_html_to_plaintext</a> is enabled and the site is configured to allow these conversions.') +
	    '</p>',
            ...
            ('filter_mime_types', mm_cfg.Text, (10, WIDTH), 0,
             _("""Remove message attachments that have a matching content type."""),

             _("""Use this option to remove each message attachment that matches one of these content types. Each line should contain a string naming a MIME <tt>type/subtype</tt>, e.g. <tt>image/gif</tt>. Leave off the subtype to remove all parts with a matching major content type, e.g. <tt>image</tt>.""") + '</p>' +

             '<p>' + _('Blank lines are ignored.') + '</p>' +

             '<p>' + _('See also <a href="?VARHELP=contentfilter/pass_mime_types">pass_mime_types</a> for a content type whitelist.')),
            ...
            ('pass_mime_types', mm_cfg.Text, (10, WIDTH), 0,
             ...
             >filter_mime_types</a>.</p>

             <p><strong>Note:</strong> if you add entries to this list but don't add
