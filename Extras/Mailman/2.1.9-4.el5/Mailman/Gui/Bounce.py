...
class Bounce(GUIBase):
    ...
    def GetConfigInfo(self, mlist, category, subcat=None):
	    ...
	    '\n</p>' +
            '\n<p>' + 
	    _('When a bounce is received, Mailman tries to extract two pieces of information from the message: the address of the member the message was intended for, and the severity of the problem causing the bounce. The severity can be either <em>hard</em> or <em>soft</em> meaning either a fatal error occurred, or a transient error occurred. When in doubt, a hard severity is used.') +
	    '\n</p>' +
            '\n<p>' + 
            _('If no member address can be extracted from the bounce, then the bounce is usually discarded. Otherwise, each member is assigned a <em>bounce score</em> and every time we encounter a bounce from this member we increment the score. Hard bounces increment by 1 while soft bounces increment by 0.5. We only increment the bounce score once per day, so even if we receive ten hard bounces from a member per day, their score will increase by only 1 for that day.') +
	    '\n</p>' +
	    '\n<p>' +
            _("""When a member's bounce score is greater than the <a href="?VARHELP=bounce/bounce_score_threshold">bounce score threshold</a>, the subscription is disabled. Once disabled, the member will not receive any postings from the list until their membership is explicitly re-enabled (either by the list administrator or the user). However, they will receive occasional reminders that their membership has been disabled, and these reminders will include information about how to re-enable their membership.""") +
	    '\n</p>' +
	    '\n<p>' +
            _("""You can control both the <a href="?VARHELP=bounce/bounce_you_are_disabled_warnings">number of reminders</a> the member will receive and the <a href="?VARHELP=bounce/bounce_you_are_disabled_warnings_interval">frequency</a> with which these reminders are sent.""") +
	    '\n</p>' +
	    '\n<p>' +
	    ...
	    '\n</p>',
            ...
            ('bounce_score_threshold', mm_cfg.Number, 5, 0,
             ...
             <p>This variable describes the upper limit for a member's bounce
            ...
            ('bounce_unrecognized_goes_to_list_owner', mm_cfg.Toggle,
             ...
             so that this new format can be added to its known set.</p>

             <p>If you really can't be bothered, then set this variable to
             <em>No</em> and all non-detected bounces will be discarded
             without further processing.</p>

             <p><strong>Note:</strong> This setting will also affect all messages sent
            ...
            ('bounce_notify_owner_on_disable', mm_cfg.Toggle,
             ...
             _("""By setting this value to <em>No</em>, you turn off notification messages that are normally sent to the list owners when a member's delivery is disabled due to excessive bounces. An attempt to notify the member will always be made.""")),

            ('bounce_notify_owner_on_removal', mm_cfg.Toggle,
             (_('No'), _('Yes')), 0,
             _("""Should Mailman notify you, the list owner, when bounces
             cause a member to be unsubscribed?"""),
             _("""By setting this value to <em>No</em>, you turn off notification messages that are normally sent to the list owners when a member is unsubscribed due to excessive bounces. An attempt to notify the member will always be made.""")),
