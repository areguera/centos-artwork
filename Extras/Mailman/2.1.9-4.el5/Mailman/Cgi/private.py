...
def main():
    ...
    if not parts:
	title = 'Private Archive Error'
        doc.SetTitle(title)
	doc.addError(_('You must specify a list.'), title)
    ...
    if tpath <> path[1:]:
        title = _('Private archive - "./" and "../" not allowed in URL.')
        doc.SetTitle(title)
        doc.addError(title)
	doc.AddItem(MailmanLogo())
        print doc.Format()
    ...
    if not mlist.WebAuthenticate((mm_cfg.AuthUser,
        ...
        if cgidata.has_key('submit'):
            ...
            message = Div(Paragraph(_('Authorization failed.'))).Format(css='class="message error strong"')
    ...
    try:
        ...
        title = _('Private Archive Error')
        doc.SetTitle(title)
	doc.addError(_('Private archive file not found'), title)
	doc.addItem(MailmanLogo())
        print doc.Format()
