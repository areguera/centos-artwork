...
************************************************************************/?>
	...
	<div class="block lexique">
		<h2 class="letter"><span><?php lexique::letter() ?> - <?php lexique::numWords() ?></span></h2>
		<div class="box">
			<div class="inbox">
			<?php if (!$words->isEmpty()) : ?>
			<dl>
			<?php while ($words->fetch()) : ?>
			<dt class="word<?php lexique::wordStatusClass(' inactive', ' active') ?>">
				<?php lexique::word() ?></dt>
			<dd class="word_def<?php lexique::wordStatusClass(' inactive', ' active') ?>">
				<?php lexique::wordDefinition() ?></dd>
			<?php lexique::wordExample('<dd class="word_example'.lexique::wordStatusClass(' inactive', ' active', true).'">%s</dd>') ?>

			<?php if (lexique::wordEditor()) : ?>
				<dd class="word_infos">
					<?php tpl::lang('Added by'); ?> 
					<a href="<?php tpl::url('forums') ?>profile.php?id=<?php lexique::wordUserId() ?>">
