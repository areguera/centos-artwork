...
# Retrieves absolute pahts from source files and checks their
# existence.
#
# In order for design templates to point different artistic motifs,
# design templates make use of external files which point to specific
...
# higher background image (e.g., 2048x1536.png).  If this isn't
# possible either, then create the background image using a plain
# color and crop from it then.  We can't go on without the required
...
function svg_checkModelAbsref {
    ...
    for BG_DST_FILE in ${BG_DST_FILES};do
        ...
        # exist. In this case, tcar.sh script will end up with
