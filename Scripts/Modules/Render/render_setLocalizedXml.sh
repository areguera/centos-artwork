...
# Standardize the way (.po) translation files are applied to XML files
# (e.g., .docbook, .svg) in order to produce their related translated
# instances, used to expand translation markers and produce the final
# file format in different languages.  Assuming no translation file
# exists, an untranslated instance is taken from the design model and
# created (i.e., just a copy) from it.  Using a design model instance
